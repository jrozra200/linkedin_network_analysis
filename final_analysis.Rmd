...
mes <- read.csv("data/raw/messages.csv")
```

# Total Connections Over Time

I currently have **`r dim(dat)[1]`** connections. My first connection was made 
on October 30, 2008 to [Leigh Sauter](https://www.linkedin.com/in/lhsauter). We 
were colleagues at The Greater Philadelphia Chamber of Commerce... well, I was 
an intern and she was an event coordinator. 
...
, I connected with **`r max(plot_dat$connections)`** people! This was the 
summer after I started my first professional job and was doing some soul 
searching on the meaning of life and what I want to do with it. 
...
dat$Company <- ifelse(grepl("Comcast", dat$Company), "Comcast", 
                      dat$Company)

dat$Company <- ifelse(grepl("Greater Philadelphia Chamber of Commerce", 
                            dat$Company), 
...
A significant chunk of people I am connected to on LinkedIn are from places 
where I work or have worked (Comcast, Socure, CivicActions, Booz Allen 
Hamilton, and Villanova). With the lionshare of this coming from Comcast, 
**`r percent(sum(companies$people[companies$Company %in% c("Comcast", "Socure", "CivicActions", "Booz Allen Hamilton", "The Chamber of Commerce for Greater Philadelphia", "Villanova University")]) / dim(dat)[1], accuracy = 0.01)`**
of my total connections are from my previous workplaces. Granted, more people 
than this are my former colleagues, they just now work at some new 
establishment that I've yet to work.
